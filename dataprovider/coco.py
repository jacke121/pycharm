...
from dataprovider.database import Dataset
...
class CocoDataset(Dataset):
    ...
    def load_image(self, image_id, image_size=None):
        img_file = self.image_info[image_id]['path']
    ...
    def load_segmentation(self, image_id, image_size=None):
        ...
        if image_size:
            true_mask = np.zeros((image_size[1], image_size[0]), dtype=np.uint8)
        else:
        ...
        for i, class_id in enumerate(class_ids):
            tmp_mask = mask[:, :, i]
            if image_size:
                img = Image.fromarray(tmp_mask)
                tmp_mask = img.resize(image_size)
                tmp_mask = np.asarray(tmp_mask, np.uint8)
            true_mask[true_mask == 0] += np.abs(class_ids[i]) * tmp_mask[true_mask == 0]
        final_mask = np.zeros(true_mask.shape + (self.num_classes,), dtype=np.uint8)
        for i in range(final_mask.shape[0]):
            for j in range(final_mask.shape[1]):
                final_mask[i, j, true_mask[i, j]] = 1
